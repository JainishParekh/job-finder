...
async def handle_cookie_consent(page):
    ...
    try:
        ...
        accept_button = page.locator(
            "button#uc-btn-accept-banner, button[data-testid='uc-accept-all-button']"
        )
        ...
        try:
            ...
            for raw_url in job_links:
                ...
                existing = (
                    supabase.table("jobs")
                    .select("id")
                    .eq("external_job_id", external_id)
                    .execute()
                )
                ...
                await job_page.route(
                    "**/*.{png,jpg,jpeg,gif,svg,woff,woff2}",
                    lambda route: route.abort(),
                )

                print(f"🎈 No, existing data : {len(existing.data)}")
                ...
                try:
                    ...
                    app_data = {"job_id": job_uuid, "status": "DISCOVERED"}
